...
BREAKPOINT {  
    ...
    if (tau <= 0) {  x = m + s  * erand() }  : remember: true "white-noise" is impossible to generate anyway..
...
if (tau > 0) {  x = x + (1. - exp(-dt/tau)) * (m - x) + sqrt(1.-exp(-2.*dt/tau)) * s  * erand()}
:printf("nornrand = %g " ,erand())
}
